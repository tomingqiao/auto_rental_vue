        ...
        <el-main>
            <el-form :model="dateModel" size="small" inline>
                <el-form-item label="日期:">
                    <el-date-picker v-model="dateModel.date" type="date" placeholder="选择日期" @change="updateData"
                        value-format="yyyy-MM-dd"></el-date-picker>
                </el-form-item>
                <el-form-item label="当日应收金额:">
                    <el-tag type="success">{{ rentPayable }}</el-tag>
                </el-form-item>
                <el-form-item label="当日实收金额:">
                    <el-tag type="info">{{ rentActual }}</el-tag>
                </el-form-item>
                <el-form-item label="当日收到押金:">
...
export default {
    ...
    methods: {
        async getData() {
            const res = await financeApi.countDay(this.dateModel.date);
            if (res.success) {
                this.hours = res.data[0];
                this.rentalData = res.data[1];
                this.returnData = res.data[2];
            }
            const resCost = await financeApi.countDayCost(this.dateModel.date);
            if (resCost.success) {
        ...
        async updateData() {
            ...
            this.chart.setOption({
                title: {
                    subtext: new Date(this.dateModel.date).toLocaleDateString()
                },
                xAxis: {
                    data: this.hours
                },
